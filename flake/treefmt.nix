    ...
    treefmt = {
      ...
      programs = {
        ...
        mdformat = {
          ...
          #
          # CHANGELOG.md is generated by `cz bump`, which writes the file and
          # commits it in one step. Formatting it rewrites the file mid-commit,
          # so the pre-commit treefmt hook fails on its own change and takes the
          # release with it. A generator owns the shape of its output.
          excludes = [
            "docs/**"
            "**/skills/**"
            "CHANGELOG.md"
